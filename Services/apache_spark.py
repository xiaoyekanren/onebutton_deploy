...
cf = fabfile.cf
# 定义env
env.user = cf.get('spark', 'localuser')
env.password = cf.get('spark', 'localuser_passwd')
env.hosts = cf.get('spark', 'hosts').split(',')
# 定义sudo用户参数
sudouser = cf.get('spark', 'sudouser')
sudouser_passwd = cf.get('spark', 'sudouser_passwd')
# 定义软件参数
